...
//process.title等于就是命令发布  fis release -wc [ D:/meiyuefuApp/meiyuefu_data_test2/admin-stat ]
//process.title.split(/\s/)将字符串分割成数组 按正则分割空格
//process.title.split(/\s/)[3]取索引为3的数组
//['sandbox','publish'].indexOf(process.title.split(/\s/)[3])前两个数组进行查找匹配索引，如果右侧字符串为publish，
//isPublish=1，1!=-1,结果再做个判断如果不等于-1，isPublish等于true
var isPublish = ['sandbox','publish'].indexOf(process.title.split(/\s/)[3]) != -1;

//如果是publish命令修改staticsPath，pagePrePath路径
if (isPublish) {
